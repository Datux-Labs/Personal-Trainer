# Run 4 â€” cross-engine accessible-name divergence
...
thing ADR 0010 is guarding against â€” an in-page SDK inventing names that nothing else
agrees with â€” is a much larger effect than the browser you happen to measure from.
...
read WebKit's own accessibility tree (see Â§4). So this is Blink vs Gecko, two of three
...
|---|---|---|---|
| `gecko-only-name` | 22 | 1.8â€“1.9% | Hacker News upvote arrows. Blink computes `""`; Gecko computes `"upvote"` from a `title` attribute on a descendant of the link. Gecko walks to a descendant `title`, Blink does not. |
...
9 of 10 sites agree perfectly. The divergence is not spread thinly across the web â€” it is
...
â€” BBC News shared zero comparable elements, Tailwind read 66%.
...
**Generalising Â§9's two-runs rule:** repetition alone would not have caught this. Two live
...
The frozen-snapshot method strips JavaScript, so every number in Â§1 describes a **static
...
`page.accessibility.snapshot()` â€” which reads each engine's own accessibility tree â€” **was
...
in complete agreement, ADR 0010 needs no browser dimension" â€” the right answer, arrived at
...
## 5. Authenticated interiors â€” and conformance measures the wrong thing there
...
on their own login page for open trial â€” OrangeHRM's open-source demo and Sauce Labs' demo
store â€” across five interior states, ~290 elements, three runs. Nothing was created, edited
...
naive. The jump happens almost entirely at rung v5 â€” CSS generated content â€” because the
...
| names containing **no letter or digit at all** | 221 â€” **68.6%** |
| names containing a **Private Use Area glyph** | 223 â€” **69.3%** |

More than two thirds of the accessible names in this admin interface are icon-font glyphs.
`<button>` elements whose computed name is a single character in the Private Use Area â€” for
example U+F284 â€” which is meaningless to a human, to a screen-reader user, and to any
...
`instrumentation.md` Â§3's still-unevidenced assertion that poor accessibility semantics and
...
`"Name (A to Z)Name (Z to A)Price (low to high)â€¦"`; Chromium correctly gives the element no
name at all. A reimplementation does not only miss names â€” it also **invents** them, and an
...
â€” which is the `miss:unknown-source` class from run 3 that never went away.

**Freezing the target changed the headline by 6 points and deleted a whole divergence
class.** I have now been caught twice by live-page variance â€” once in run 3 with a
...
## 7. Corrections to Â§9

**Add â€” pin the target for any comparative measurement.** Two runs catches instability; it
does not catch a confound that is stable in both arms. Anything comparing tools, engines or
time must serve both arms identical bytes. This run's headline moved 92% â†’ 98% on that
change alone.

**Add â€” for any measurement of accessible names, record which component computed them.**
Engine, driver, or test library. A library-computed name will manufacture agreement and the
result will look clean.

**Sharpen â€” "authenticated interiors" is not the right criterion.** What matters is
...
**Retire â€” the worry that landing-page corpora are optimistic.** Measured, and the effect
...
- **Gecko was Playwright's patched Firefox build**, driven by geckodriver. It is a real
  ...
  invisible to me.
- **Static DOM only** â€” scripts stripped, see Â§3.
...
*L2 experiment record. Rig in [`tools/`](../tools) â€” `crossengine.js` is new in this run and
